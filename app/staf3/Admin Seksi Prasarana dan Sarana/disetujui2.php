...
if (isset($_GET['NIP']) && isset($_GET['Pegawai'])) {
    ...
    $query = mysqli_query($koneksi, "SELECT tgl_permintaan, jam, Pegawai, NIP, NamaPegawai, tglManual, count(id_permintaan)  FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE NIP='$NIP' AND Pegawai='$Pegawai' AND Submit='berhasil' AND status=1 GROUP BY tglManual DESC ");   
}
?>
