...
$query = mysqli_query($koneksi, "SELECT distinct(NIP), Pegawai, NamaPegawai, tgl_permintaan, jam, tglManual, penempatan, level, Akses, count(tglManual) FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE Akses='Seksi Prasarana dan Sarana' AND status=1 AND Submit='berhasil' GROUP BY NIP"); 
?>
<body>
