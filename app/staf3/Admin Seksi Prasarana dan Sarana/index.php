...
$query2 = mysqli_query($koneksi, "SELECT NIP, tgl_permintaan, Akses, Keterangan, count(status) AS jumlah2 FROM permintaan WHERE status=0 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana'"); 
			...
			<nav class="navbar navbar-header navbar-expand-lg" data-background-color="blue">
				<div class="container-fluid">
					<ul class="navbar-nav topbar-nav ml-md-auto align-items-center">
						<?php
						include '../../../fungsi/koneksi.php';
						$sql = 'SELECT NIP, Akses, penempatan, jam, count(NIP) FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE status=0 AND submit="berhasil" AND Akses="Seksi Prasarana dan Sarana" GROUP BY NIP DESC limit 5';
						$query = mysqli_query($koneksi, $sql);
						$row = mysqli_fetch_array($query);
						$query2 = mysqli_query($koneksi, "SELECT NIP, tgl_permintaan, Akses, Keterangan, count(status) AS jumlah2 FROM permintaan WHERE status=0 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana'"); 
						$data2 = mysqli_fetch_assoc($query2);
						$query3 = mysqli_query($koneksi, "SELECT gambar, NIP, tgl_permintaan, Akses, Keterangan, count(status) AS jumlah3 FROM permintaan WHERE status=0 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana'"); 
							...
							<ul class="nav nav-collapse">
								<li>
									...
									<a href="index.php?p=userStaff">		
										<span class="sub-item">Data Pengguna Seksi Prasarana dan Sarana</span>
									</a>
								</li>
							</ul>
						</div>
...
<script>
		...
		<?php
		include "../../../fungsi/koneksi.php";
		$tunggu       = mysqli_query($koneksi, "SELECT count(status) AS jumlah0 FROM permintaan where status=0  AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana'");
		$tolak       = mysqli_query($koneksi, "SELECT count(status) AS jumlah1 FROM permintaan where status=2 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana' ");
		$terima       = mysqli_query($koneksi, "SELECT count(status) AS jumlah2 FROM permintaan where status=1 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana' ");
		?>
		...
		<?php
		include "../../../fungsi/koneksi.php";
		$tunggu1       = mysqli_query($koneksi, "SELECT count(status) AS jumlah01 FROM permintaan where status=0 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana' ");
		$tolak1       = mysqli_query($koneksi, "SELECT count(status) AS jumlah11 FROM permintaan where status=2 AND submit='berhasil' AND Akses='Seksi Prasarana dan Sarana' ");
		$terima1       = mysqli_query($koneksi, "SELECT count(status) AS jumlah21 FROM permintaan where status=1  AND submit='berhasil'AND Akses='Seksi Prasarana dan Sarana' ");
		?>
