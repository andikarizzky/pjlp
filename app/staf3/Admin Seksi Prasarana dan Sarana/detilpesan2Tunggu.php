...
if (isset($_GET['NIP']) && isset($_GET['Pegawai'])) {
    ...
    $query = mysqli_query($koneksi, "SELECT tgl_permintaan, NIP, Pegawai, jam, tglManual, count(id_permintaan)  FROM permintaan WHERE NIP='$NIP' AND Pegawai='$Pegawai' AND Submit='berhasil' AND status=0  GROUP BY tglManual DESC ");   
}
?>
                                    ...
                                    <tr>
                                        ...
                                        if (mysqli_num_rows($query)) {
                                            while($row=mysqli_fetch_assoc($query)):
                                               ...
                                               <td>                                         
                                                <a href="?p=detil3Tunggu&tgl=<?= $row['tglManual']; ?>&NIP=<?= $row['NIP']; ?>&Pegawai=<?= $row['Pegawai']; ?>"><span data-placement='top' data-toggle='tooltip' title='Detail Laporan'><button    class="btn btn-info btn-round btn-icon"><i class="fas fa-eye"></i></button></span></a>        
                                            </td>
                                        </tr>   
