...
if (isset($_GET['aksi']) && isset($_GET['id'])) {
    ...
    if ($_GET['aksi'] == 'edituserPerorangan') {
        header("location:?p=edituserPerorangan&id=$id");
    } else if ($_GET['aksi'] == 'hapus') {
        header("location:?p=hapususer&id=$id");
    } 
}
$query = mysqli_query($koneksi, "SELECT * FROM user WHERE level!='Sub Bagian Tata Usaha' AND level!='Admin Sub Bagian Tata Usaha' AND level!='Seksi Pelayanan dan Informasi' AND level!='Admin Seksi Pelayanan dan Informasi' AND level!='Seksi Prasarana dan Sarana' AND level!='Admin Seksi Prasarana dan Sarana' AND level!='Admin Seksi Konservarsi Peragaan Penelitian dan Pengembangan' AND level!='IT' ORDER BY level DESC");	
?>
                                ...
                                <tr>
                                    ...
                                    if (mysqli_num_rows($query)) {
                                        while($row=mysqli_fetch_assoc($query)):
                                           ...
                                           <td class="text-primary"><?= $row['level']; ?></td>
                                           <td class="text-info"><?= $row['penempatan']; ?></td>
                                           <td>
                                        ...
                                        <td>
                                            <a href="?p=edituserPerorangan&id=<?= $row['id_user']; ?>">
                                                <button type="button" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-original-title="Edit Perorangan">
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="#delPerorangan<?php echo $row['id_user']; ?>" data-toggle="modal">
                                               <button type="button" data-toggle="tooltip" title="" class="btn btn-link btn-danger" data-original-title="Remove">
                                                <i class="fas fa-eraser"></i>
                                            </button></a>
                                            <?php include('buttonPerorangan.php'); ?>  
                                        </td>
                                    </tr>
                                    <?php $no++; endwhile; } ?>
                                </tbody>
