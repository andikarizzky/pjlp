...
$query = mysqli_query($koneksi, "SELECT distinct(NIP), NamaPegawai, Pegawai, NIP, AtasanSatu, tgl_permintaan, jam, tglManual, penempatan, level, Akses, count(tglManual) FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE Akses!='Sub Bagian Tata Usaha' AND Akses!='Admin Sub Bagian Tata Usaha' AND Akses!='Seksi Pelayanan dan Informasi' AND Akses!='Admin Seksi Pelayanan dan Informasi' AND Akses!='Seksi Prasarana dan Sarana' AND Akses!='Admin Seksi Prasarana dan Sarana' AND Akses!='Admin Seksi Konservarsi Peragaan Penelitian dan Pengembangan' AND Submit='berhasil' AND status=2 GROUP BY NIP"); 
        ...
        <div class="col-md-12">
            <div class="card">           
                <div class="card-header">
                    <div class="card-title">Data Laporan Tolak Seksi Konservarsi Peragaan Penelitian dan Pengembangan</div>
                </div>
                <div class="card-body">
                                    ...
                                    <tr>
                                        ...
                                        if (mysqli_num_rows($query)) {
                                            while($row=mysqli_fetch_assoc($query)):
                                               ...
                                               <td>                                                 <a data-original-title="Lihat"  data-toggle="tooltip" href="?p=detilpesan2Tolak&NIP=<?= $row['NIP'];?>&Pegawai=<?= $row['Pegawai'];?>"><button type="button" class="btn btn-icon btn-round btn-info">
                                                <i class="fas fa-eye"></i>    
                                            </button>
                                        </a>            
                                    </td>                           
                                </tr>
                                <?php $no++; endwhile; }else {echo "<tr><td colspan=9>Belum ada Laporan Tolak.</td></tr>";} ?>
                            </tbody>
                        </table>
                    </div>                  
                </div>
