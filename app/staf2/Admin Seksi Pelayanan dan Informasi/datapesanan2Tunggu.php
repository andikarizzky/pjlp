...
$query = mysqli_query($koneksi, "SELECT distinct(NIP), NamaPegawai, Pegawai, AtasanSatu, tgl_permintaan, jam, tglManual, penempatan, level, Akses, count(tglManual) FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE Akses='Seksi Pelayanan dan Informasi' AND Submit='berhasil' AND status=0 GROUP BY NIP"); 
?>
<body>
        ...
        <div class="col-md-12">
            <div class="card">           
                <div class="card-header">
                    <div class="card-title">Data Laporan Tunggu Seksi Pelayanan dan Informasi</div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">								
                        <div class="table-responsive">
                            <table  id="basic-datatables" class="display table table-striped table-hover table-head-bg-primary">
                                ...
                                <?php $no++; endwhile; }else {echo "<tr><td colspan=9>Belum ada Laporan Di Tolak.</td></tr>";} ?>
                            </tbody>
                        </table>
                    </div>                  
                </div>
