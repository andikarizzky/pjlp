...
if (isset($_GET['tgl'])) {
    $tgl = $_GET['tgl'];
    $query = mysqli_query($koneksi, "SELECT penempatan, jamManual2, status, catatan, NIP, jamManual, Akses, Pegawai, Keterangan, tgl_permintaan FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE  status=1 AND Akses='Seksi Pelayanan dan Informasi' AND tglManual='$tgl' ORDER BY NIP DESC ");    
}
?>
