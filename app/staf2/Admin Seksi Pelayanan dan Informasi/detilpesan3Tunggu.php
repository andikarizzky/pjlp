...
if (isset($_GET['tgl']) && isset($_GET['NIP']) && isset($_GET['Pegawai'])) {
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM permintaan WHERE tglManual='$tgl' AND NIP='$NIP' AND Pegawai='$Pegawai' AND status=0 AND Submit='berhasil' ORDER BY tglManual DESC ");    
}
?>
                ...
                <div class="card">
                 ...
                 <div class="card-body">
                    <div class="table-responsive">
                        <form name="bulk_action_form" action="delete2.php" method="post" onSubmit="return delete_confirm();"/>
                                ...
                                <tr>
                                    ...
                                    if (mysqli_num_rows($query)) {
                                        while($row=mysqli_fetch_array($query)):
                                           ?>
                                           <td align="center"><input type="checkbox" name="checked_id[]" class="checkbox" value="<?php echo $row['id_permintaan']; ?>"/></td>        
                                           <td> <?= $no; ?> </td>
                                           <td><img src="../../staf2/gambar/<?php echo $row['gambar'];?>" height="50" width="100" alt="Tidak Ada Foto"></td>
                                           <td> <?= $row['catatan']; ?> </td>
                                    ...
                                    <td>
                                        <a href="#editS<?php echo $row['id_permintaan']; ?>" data-toggle="modal"><button type="button" data-toggle="tooltip" title="" class="btn btn-link btn-success " data-original-title="Setuju"><i class="fas fa-check-square"></i></button></a><?php include('buttonS.php'); ?> <br>
                                                    <!--
                                            ...
                                            <td>
                                                <a href="#edit<?php echo $row['id_permintaan']; ?>" data-toggle="modal">
                                                    ...
                                                    <?php include('buttonT.php'); ?> <br>
                                                </td>
                                            </tr>
                                            <?php $no++; endwhile; }else {header("location:index.php?p=detil2Tunggu&NIP=$NIP&Pegawai=$Pegawai");} ?>
                                        </tbody>
                                    </table>
                                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button type="button"  class="btn btn-success"  data-toggle="modal" data-target="#setujui">Setujui
                                        <i data-toggle="tooltip" data-original-title="Pilih Di Setujui"   class="fas fa-check-double"></i>
                                        </button><?php include('modalSetuju.php'); ?>
                                    </form>
                                </div>                  
                            </div>
                        </div>
