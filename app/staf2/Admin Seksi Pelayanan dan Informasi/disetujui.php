...
$query = mysqli_query($koneksi, "SELECT distinct(NIP), Pegawai, NamaPegawai, tgl_permintaan, jam, tglManual, penempatan, level, Akses, count(tglManual) FROM permintaan LEFT JOIN user on permintaan.NIP = user.username WHERE Akses='Seksi Pelayanan dan Informasi' AND status=1 AND Submit='berhasil' GROUP BY NIP"); 
?>
<body>
                                        ...
                                        <tr>
                                            ...
                                            if (mysqli_num_rows($query)) {
                                                while($row=mysqli_fetch_assoc($query)):
                                                   ...
                                                   <td>                 
                                                    <a data-original-title="Lihat"  data-toggle="tooltip" href="?p=disetujui2&NIP=<?= $row['NIP'];?>&Pegawai=<?= $row['Pegawai'];?>"><button type="button" class="btn btn-icon btn-round btn-info">
