                            <div class="main-panel">
                                <div class="content">
                                    ...
                                    $query = mysqli_query($koneksi, "SELECT tgl_permintaan FROM permintaan WHERE NIP= '$_SESSION[username]' AND Submit='berhasil' AND status=2 ");  
                                    $no =1 ;
                                    if (mysqli_num_rows($query)) {
                                        while($row=mysqli_fetch_assoc($query)):
                                            echo '<a href="index.php?p=datapesananTolak" data-notify="container" class="col-10 col-xs-11 col-sm-2 alert alert-danger" role="alert" data-notify-position="bottom-center" style="display: inline-block; margin: 0px auto; padding-left: 65px; position: fixed; transition: all 0.5s ease-in-out 0s; z-index: 1031; top: 107px; right: 20px;"><button class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><span data-notify="icon" class="fas fa-info"></span> <p class="text-danger">Ada Laporan Di Tolak</p>
                                            </a>';
                                            ?>   
                                            <?php $no++; endwhile; }else {echo "";} ?>
                                        </div>
                                        <footer class="footer">
